...
function StatCard({ icon: Icon, tone = 'violet', label, value }) {
  ...
  return (
    <div
      className={`${currentTheme.bg} rounded-[2rem] p-6 lg:p-8 ${currentTheme.shadow} border ${tone === 'blue' ? 'border-neutral-100' : 'border-transparent'} flex flex-col justify-between relative overflow-hidden group transition-transform hover:-translate-y-1`}
    >
      {tone !== 'blue' && (
      ...
      <div
        className={`flex h-14 w-14 items-center justify-center rounded-2xl border mb-6 relative z-10 shadow-inner-soft ${currentTheme.icon}`}
      >
        <IconComponent className="h-6 w-6" />
      </div>
      <div>
        <div
          className={`text-[11px] font-bold uppercase tracking-widest opacity-80 mb-2 ${currentTheme.text}`}
        >
          {label}
        </div>
        <div
          className={`font-display text-4xl md:text-5xl font-black tracking-tight relative z-10 ${currentTheme.text}`}
        >
          {value}
        </div>
      </div>
...
export default function Evolution() {
  ...
  const engagementPath = useMemo(
    () => buildSmoothCurve(engagementSeries, 700, 240),
    [engagementSeries],
  )
  const engagementArea = useMemo(
    () => buildAreaPath(engagementSeries, 700, 240),
    [engagementSeries],
  )
  const yAxisLabels = useMemo(() => buildYAxisLabels(engagementSeries), [engagementSeries])

  const masteredSeries = useMemo(
    () => buildSnapshotSeries(dailyStats, buildRange(14)),
    [dailyStats],
  )
  ...
  const learnedWords = words.filter((word) =>
    ['reconhecida', 'em_treino', 'ativa', 'dominada'].includes(word.status),
  ).length
  ...
  const skillAxes = SKILL_LABELS.map((label, index) =>
    buildSkillAxis(label, index, SKILL_LABELS.length),
  )
            ...
            <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100 flex flex-col relative overflow-hidden">
              <div className="mb-8 flex items-start justify-between gap-4 relative z-10">
                <div>
                  ...
                  <h3 className="text-2xl lg:text-3xl font-extrabold tracking-tight text-neutral-900">
                    Consistência diária nos últimos 30 dias
                  </h3>
                </div>
                <div className="rounded-xl bg-violet-50 px-4 py-2 text-xs font-bold text-violet-700 border border-violet-100 shadow-inner-soft whitespace-nowrap hidden sm:block">
                  Mês Atual
                </div>
              </div>
            ...
            <div className="grid gap-8 lg:grid-cols-2">
              <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100 flex flex-col">
                ...
                <div className="flex justify-center flex-1 items-center">
                  <svg
                    viewBox="0 0 280 280"
                    className="h-[260px] w-[260px] overflow-visible drop-shadow-sm"
                  >
                    ...
                    {skillAxes.map((axis) => (
                      <g key={axis.label}>
                        ...
                        <text
                          ...
                          className="fill-neutral-500 text-[10px] uppercase font-bold tracking-wider"
                        >
                          {axis.label}
                        </text>
                      </g>
                    ...
                    return (
                      <div key={item.label} className="group">
                        <div className="mb-2 flex items-center justify-between text-sm">
                          <span className="font-bold text-neutral-600 uppercase tracking-widest text-[10px]">
                            {item.label}
                          </span>
                          <strong className="font-black text-neutral-900 bg-neutral-100 px-2 py-0.5 rounded-md border border-neutral-200 shadow-inner-soft text-xs">
                            {item.value}
                          </strong>
                        </div>
                        <div className="h-5 overflow-hidden rounded-full bg-neutral-100 border border-neutral-200 shadow-inner-soft">
                          <div
                            className={`h-full rounded-full transition-all duration-1000 ease-out ${toneColor} relative overflow-hidden`}
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100">
              ...
              <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-violet-100 before:z-0">
                <div
                  className={`relative z-10 rounded-[1.5rem] border p-5 shadow-sm transition-transform hover:-translate-y-1 ${words.length > 0 ? 'border-emerald-200 bg-emerald-50' : 'border-neutral-100 bg-white'}`}
                >
                  <div
                    className={`text-lg font-extrabold mb-1 ${words.length > 0 ? 'text-emerald-700' : 'text-neutral-900'}`}
                  >
                    Primeiras palavras
                  </div>
                  <div className="text-xs font-bold text-neutral-500 uppercase tracking-widest">
                    {words.length > 0
                      ? `${words.length} itens adicionados`
                      : 'Adicione vocabulário'}
                  </div>
                  {words.length > 0 && (
                    <div className="absolute -left-2 sm:-left-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-emerald-100 border-4 border-white rounded-full flex items-center justify-center shadow-sm">
                ...
                <div
                  className={`relative z-10 rounded-[1.5rem] border p-5 shadow-sm transition-transform hover:-translate-y-1 ${streak.currentStreak > 0 ? 'border-orange-200 bg-orange-50' : 'border-neutral-100 bg-white'}`}
                >
                  <div
                    className={`text-lg font-extrabold mb-1 ${streak.currentStreak > 0 ? 'text-orange-700' : 'text-neutral-900'}`}
                  >
                    Ofensiva iniciada
                  </div>
                  <div className="text-xs font-bold text-neutral-500 uppercase tracking-widest">
                    {streak.currentStreak > 0
                      ...
                      : 'Ative sua primeira streak'}
                  </div>
                  {streak.currentStreak > 0 && (
                    <div className="absolute -left-2 sm:-left-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-orange-100 border-4 border-white rounded-full flex items-center justify-center shadow-sm">
                ...
                <div
                  className={`relative z-10 rounded-[1.5rem] border p-5 shadow-sm transition-transform hover:-translate-y-1 ${achievements.length > 0 ? 'border-violet-200 bg-violet-50' : 'border-neutral-100 bg-white'}`}
                >
                  <div
                    className={`text-lg font-extrabold mb-1 ${achievements.length > 0 ? 'text-violet-700' : 'text-neutral-900'}`}
                  >
                    Conquista liberada
                  </div>
                  <div className="text-xs font-bold text-neutral-500 uppercase tracking-widest">
                    {achievements.length > 0
                      ? `${achievements.length} emblemas`
                      : 'Cumpra desafios'}
                  </div>
                  {achievements.length > 0 && (
                    <div className="absolute -left-2 sm:-left-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-violet-100 border-4 border-white rounded-full flex items-center justify-center shadow-sm">
            ...
            <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-[1.5rem] border border-neutral-100 bg-neutral-50/50 p-5 shadow-inner-soft hover:shadow-soft transition-shadow">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-orange-600 mb-2">
                    Retenção semanal
                  </div>
                  <div className="text-3xl font-black tracking-tight text-neutral-900">
                    {retention.rate}%
                ...
                <div className="rounded-[1.5rem] border border-neutral-100 bg-neutral-50/50 p-5 shadow-inner-soft hover:shadow-soft transition-shadow">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-pink-600 mb-2">
                    Dominadas 14d
                  </div>
                  <div className="text-3xl font-black tracking-tight text-neutral-900">
                    {masteredSeries.at(-1) ?? 0}
                  </div>
                </div>
              </div>
            </div>
