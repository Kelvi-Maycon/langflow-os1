...
const ACHIEVEMENT_RULES = [
  ...
  },
  {
    key: 'weekly_rhythm',
    goal: 4,
    getCurrent: ({ dailyStats, minSessionMinutes }) =>
      getStudyDaysInLast(7, dailyStats, minSessionMinutes),
...
export function buildPromptCardState({ promptTargets = [], pendingPrompt = false } = {}) {
  ...
  if (!pendingPrompt) {
    return {
      ...
      description:
        'Voce ja fechou o desafio de escrita de hoje e pode revisar ou capturar mais contexto.',
...
export function buildAchievementSpotlight({
  ...
  if (!nextLocked) {
    return {
      ...
      description:
        'Voce completou todos os marcos atuais e pode continuar subindo seus numeros diarios.',
...
export function getAutoAdjustSummary({ enabled, autoAdjustMeta, currentLevel }) {
  if (!enabled) {
    return {
      ...
      description:
        'Ative o ajuste automatico nas configuracoes para recalibrar a complexidade com base no seu desempenho.',
    }
  }
  ...
  return {
    ...
    title: `Nivel atual ${currentLevel || 'B1'}`,
    description:
      'Complete mais exercicios guiados para liberar a primeira recomendacao automatica de nivel.',
  }
}
