...
function getAnswerFeedback(answerTokens, expectedTokens) {
  return expectedTokens.map(
    (expected, index) => answerTokens[index]?.text?.toLowerCase() === expected.text.toLowerCase(),
  )
}
...
function SentenceExercise({ exercise, onComplete, onSave }) {
  ...
  const [availableTokens, setAvailableTokens] = useState(() =>
    createShuffledTokenItems(exercise.sentence.id, exercise.sentence.english),
  )
  ...
  const checkAnswer = () => {
    ...
    const correct =
      normalizeEnglishAnswer(answer) === normalizeEnglishAnswer(exercise.sentence.english)
  ...
  const handleDragEnd = ({ active, over }) => {
    ...
    const targetContainer =
      over.id === AVAILABLE_CONTAINER_ID || over.id === ANSWER_CONTAINER_ID
        ? over.id
  ...
  const answerFeedback =
    result === 'try_again' || result === 'incorrect'
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-soft border border-neutral-100 flex flex-col gap-6 relative overflow-hidden">
      <div className="bg-violet-50 p-5 rounded-2xl border border-violet-100/50 mb-2 relative overflow-hidden">
        ...
        <div className="text-xl md:text-2xl font-bold text-violet-950 leading-tight">
          {exercise.sentence.portuguese}
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
        <div className="flex flex-col gap-4">
          <label className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest px-2">
          ...
          >
            ...
            />
            <label className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest px-2 mt-4">
              Banco de palavras
            </label>
            <SortableLane
        ...
        <div className="bg-neutral-50/80 rounded-2xl p-6 border border-neutral-200/60 shadow-inner-soft self-stretch flex flex-col pt-5">
          ...
          <div className="text-sm text-neutral-500 bg-white p-3 rounded-xl border border-neutral-100 shadow-sm mb-3 leading-relaxed">
            Palavra foco:
            <br />
            <strong className="text-violet-700">{exercise.wordText}</strong>{' '}
            <span className="opacity-50">·</span> variação <strong>{exercise.sentence.type}</strong>
          </div>
          <div className="text-xs font-semibold text-neutral-400 mt-auto bg-neutral-200/50 p-3 rounded-xl">
            Tentativas usadas:{' '}
            <strong className={attempts >= maxAttempts ? 'text-red-500' : 'text-neutral-700'}>
              {attempts}
            </strong>{' '}
            / {maxAttempts}
          </div>
      ...
      {result === 'try_again' && (
        <div
          className="builder-feedback builder-feedback-warning mt-lg"
          style={{ background: 'rgba(245, 158, 11, 0.1)', borderColor: 'rgba(245, 158, 11, 0.2)' }}
        >
          <div className="builder-feedback-title" style={{ color: '#D97706' }}>
            ⚠️ Ainda nao ficou certo
          </div>
          <div className="builder-feedback-copy" style={{ color: '#D97706', marginTop: 4 }}>
            Revise a ordem das palavras destacadas. Você tem mais {maxAttempts - attempts}{' '}
            {maxAttempts - attempts === 1 ? 'tentativa' : 'tentativas'}.
          </div>
        </div>
      )}
...
function TransformExercise({ exercise, onComplete }) {
  ...
  const handleVerify = () => {
    ...
    const success =
      normalizeEnglishAnswer(answer) === normalizeEnglishAnswer(exercise.expectedSentence)
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-soft border border-neutral-100 flex flex-col gap-6">
      ...
      <div className="bg-neutral-50/80 rounded-2xl p-6 border border-neutral-200/60 shadow-inner-soft">
        ...
        <div className="text-lg font-bold text-neutral-800 bg-white p-4 rounded-xl border border-neutral-100 shadow-sm mb-2">
          {exercise.sourceSentence}
        </div>
        <div className="text-sm font-medium text-neutral-500 px-2 italic">
          {exercise.sourcePortuguese}
        </div>
      </div>

      <div className="mt-2">
        <label className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest px-2 mb-2 block">
          Digite a nova frase em inglês
        </label>
        <textarea
      ...
      {(!result || result === 'try_again') && (
        <div className="flex gap-sm mt-lg">
          <button className="btn btn-primary" onClick={handleVerify} disabled={!answer.trim()}>
            ✓ Verificar
          </button>
          <button className="btn btn-ghost" onClick={() => setAnswer('')}>
            Limpar
          </button>
        </div>
      )}
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-soft border border-neutral-100 flex flex-col gap-6">
      ...
      <div className="bg-neutral-50/80 rounded-2xl p-8 border border-neutral-200/60 shadow-inner-soft text-center">
        <div className="flex items-center justify-center gap-2 text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-4">
          Complete com a palavra certa
        </div>
        <div className="text-2xl font-black text-neutral-900 leading-normal">
          {exercise.maskedEnglish}
        </div>
      </div>

      <div className="mt-2 text-center max-w-lg mx-auto w-full">
        <label className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest px-2 mb-2 block">
          Palavra ou expressão faltante
        </label>
        <input
...
function DailyPromptPanel({ targets }) {
  ...
  const [feedback, setFeedback] = useState(
    existingSubmission ? { type: 'info', text: 'Prompt de hoje já concluído.' } : null,
  )
  ...
  const handleSubmit = () => {
    ...
    const invalidIndex = answers.findIndex(
      (answer, index) => !containsLexiconText(answer, targets[index]?.wordText),
    )
    if (invalidIndex !== -1) {
      setFeedback({
        type: 'warning',
  ...
  if (targets.length === 0) {
    return (
      <div className="empty-state">
        ...
        <p>
          Adicione palavras ao banco ou importe o seed do seu nível para desbloquear o desafio
          diário.
        </p>
      </div>
    )
  ...
  return (
    <div className="bg-white rounded-[2rem] p-6 lg:p-10 shadow-soft border border-neutral-100 mb-8 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-3xl font-extrabold text-neutral-900 mb-2 tracking-tight">
          Escreva 3 frases usando os termos do dia.
        </h3>
        <p className="text-sm text-neutral-500 max-w-xl mb-8">
          Essa prática vale bastante XP e consolida o vocabulário recente.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
        {targets.map((target, index) => (
          <div
            key={target.wordId}
            className="bg-neutral-50/80 rounded-2xl p-6 border border-neutral-200/60 shadow-inner-soft hover:border-violet-200 transition-colors group"
          >
            <div className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-1 group-hover:text-violet-500 transition-colors">
              Termo {index + 1}
            </div>
            <div className="text-xl font-bold text-neutral-900 group-hover:text-violet-700 transition-colors">
              {target.wordText}
            </div>
            <div className="mt-2 text-xs font-semibold text-neutral-500 bg-white px-3 py-1.5 rounded-lg border border-neutral-100 shadow-sm inline-block">
              {target.entryType === 'collocation'
                ? 'Use a expressão inteira na frase.'
                : 'Use a palavra em uma frase natural.'}
            </div>
            <textarea
...
export default function Builder({ initialWords = [], initialMode = 'assembly', onDone }) {
        ...
        void (async () => {
          for (const word of selectedWords) {
            ...
            try {
              ...
              warmControllersRef.current = warmControllersRef.current.filter(
  ...
  useEffect(() => {
    ...
    if (
      ...
      !completionFlagsRef.current.assembly
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !completionFlagsRef.current.transform
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !completionFlagsRef.current.cloze
    ) {
        ...
        <main className="max-w-[1400px] w-full mt-2 lg:mt-4 mx-auto px-4 md:px-8">
          <div className="bg-white rounded-3xl p-12 shadow-soft border border-neutral-100 flex flex-col items-center justify-center text-center mt-12">
            ...
            <p className="text-neutral-500 max-w-md">
              Montando até {builderConfig.sessionWordLimit ?? 5} palavras com{' '}
              {builderConfig.phrasesPerWord ?? 3} variações por item.
            </p>
          </div>
        </main>
      </div>
        ...
        <main className="max-w-[1400px] w-full mt-2 lg:mt-4 mx-auto px-4 md:px-8">
          {error ? (
            ...
            <div className="bg-white rounded-3xl p-12 shadow-soft border border-neutral-100 flex flex-col items-center justify-center text-center mt-12 mb-8">
              ...
              <p className="text-neutral-500 max-w-md">
                Clique em palavras no Reader, importe o seed do seu nível ou adicione itens ao banco
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            <button
              className="bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white px-8 py-3.5 rounded-full font-bold shadow-lg transition-transform hover:-translate-y-0.5 flex items-center gap-2"
              onClick={prepare}
            >
              <ReloadIcon size={18} />
              Atualizar sessão
  ...
  const progress =
    currentModeMeta.count > 0 && mode !== 'prompt'
  ...
  const currentFocus =
    currentExercise?.wordText ||
    practiceData.selectedWords
      .slice(0, 3)
  ...
  const completedCount =
    mode === 'prompt'
      ? Number(
          Boolean(
            useProgressStore.getState().dailyPromptHistory?.[
  ...
  return (
    <div className="text-neutral-800 antialiased min-h-screen flex flex-col pt-0 lg:pt-0 pb-16">
      <header className="px-4 md:px-8 h-20 w-full hidden md:flex items-center justify-between">
        <div className="flex items-center gap-8">
          <div>
            ...
            <p className="text-xs font-semibold tracking-wider text-neutral-400 uppercase mt-0.5">
              Fixação dinâmica em múltiplos formatos
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1400px] w-full mt-2 lg:mt-4 mx-auto px-4 md:px-8 relative">
        <div
          className="bg-white rounded-[2rem] p-6 lg:p-10 shadow-soft border border-neutral-100 mb-8 relative overflow-hidden"
          ref={workspaceRef}
          data-testid="builder-workspace-header"
        >
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
            <div>
              ...
              <p className="text-sm text-neutral-500 max-w-xl">
                {currentContext ||
                  'A sessão atual usa o vocabulário capturado no reader e no banco ativo.'}
              </p>
            </div>
            <div className="flex flex-wrap gap-2 lg:justify-end">
              <span className="px-3 py-1.5 bg-green-50 text-green-700 border border-green-200 rounded-lg text-[11px] font-bold uppercase tracking-wider">
                Concluídos {completedCount}
              </span>
              <span className="px-3 py-1.5 bg-neutral-100 text-neutral-600 border border-neutral-200 rounded-lg text-[11px] font-bold uppercase tracking-wider">
                Restantes {remainingCount}
              </span>
              <span className="px-3 py-1.5 bg-violet-50 text-violet-700 border border-violet-200 rounded-lg text-[11px] font-bold uppercase tracking-wider shadow-sm flex items-center gap-1.5">
                <SparkIcon size={12} /> Cards salvos {saved.length}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 pt-8 border-t border-neutral-100 relative z-10">
            <div className="bg-neutral-50/80 p-4 rounded-2xl border border-neutral-100">
              <span className="block text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5">
                Foco atual
              </span>
              <strong className="text-sm text-neutral-900 line-clamp-2">
                {currentFocus || 'Sessão sem foco definido'}
              </strong>
            </div>
            <div className="bg-neutral-50/80 p-4 rounded-2xl border border-neutral-100">
              <span className="block text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5">
              ...
              <strong className="text-2xl font-black text-neutral-900">
                {practiceData.selectedWords.length}
              </strong>
            </div>
            <div className="bg-neutral-50/80 p-4 rounded-2xl border border-neutral-100">
              <span className="block text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5">
          ...
          <div className="mb-8 pl-1">
            <div className="flex items-center justify-between mb-3 text-sm font-bold">
              <span className="text-neutral-500">
                {currentModeMeta.title} <span className="text-neutral-300 mx-1">•</span> exercício{' '}
                {Math.min(currentModeMeta.index + 1, currentModeMeta.count)} de{' '}
                {currentModeMeta.count}
              </span>
              <span className="bg-violet-100 text-violet-700 px-2 py-0.5 rounded text-[10px] uppercase tracking-wider border border-violet-200">
                {practiceData.selectedWords.length} itens na sessão
              </span>
            </div>
            <div className="h-2 w-full bg-neutral-100 rounded-full overflow-hidden border border-neutral-200/50">
              <div
                className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-400 transition-all duration-500 relative"
          ...
          (practiceData.assemblyExercises.length === 0 ? (
            ...
            <div className="page-content builder-page builder-summary-page">
              <div className="builder-summary-hero">
                <div className="builder-summary-icon">
                  <SparkIcon size={28} />
                </div>
                <h2 className="builder-summary-title">Montagem concluída</h2>
                <p className="builder-summary-copy">
                  Você completou {practiceData.assemblyExercises.length} exercícios · {saved.length}{' '}
                  frases salvas no flashcard
                </p>
              </div>
