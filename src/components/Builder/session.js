...
export function selectBuilderWords({ initialWords = [], bankWords = [], builderConfig = {} }) {
  ...
  const maxLocalWords = Math.min(
    ...
    localPool.length,
    Math.max(
      readerWords.length > 0 ? 1 : 0,
      Math.round(sessionWordLimit * (difficultWordsWeight / 100)),
    ),
  )

  const localSelection = localPool.slice(0, maxLocalWords).map((word) => ({
    wordId: word.id,
    wordText: word.word,
    originalSentence: word.originalSentence || '',
    recycled: true,
...
export function buildSentenceVariants({
  wordId,
  wordText,
  cachedSentences = [],
  phrasesPerWord = 3,
  ...
  ;[...cachedSentences, ...localFallback].forEach((sentence) => {
    ...
    const type = TYPE_ORDER.includes(sentence.type)
      ? sentence.type
      : TYPE_ORDER[byType.size] || 'positive'
    if (!byType.has(type)) {
      byType.set(type, {
        ...sentence,
  ...
  return TYPE_ORDER.filter((type) => byType.has(type))
    .slice(0, phraseLimit)
    .map((type) => byType.get(type))
}

export function buildBuilderExercises({
  selectedWords,
