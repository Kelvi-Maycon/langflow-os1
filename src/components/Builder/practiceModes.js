...
export function buildTransformExercises({ selectedWords, recentSentencesByWord, builderConfig = {} }) {
...
export function selectDailyPromptTargets({ words = [], userLevel, limit = 3 }) {
    ...
    const training = uniqueWords(words.filter((word) => ['reconhecida', 'em_treino'].includes(word.status) && !word.isSeeded));
    const seeded = uniqueWords(words.filter((word) => word.isSeeded && (!word.cefrLevel || compareCefrLevels(word.cefrLevel, userLevel) <= 0)));
    const fallback = uniqueWords(words.filter((word) => !word.isSeeded && word.status === 'desconhecida'));

    return [...active, ...training, ...seeded, ...fallback]
        .slice(0, limit)
        .map((word) => ({
            wordId: word.id,
            wordText: word.word,
