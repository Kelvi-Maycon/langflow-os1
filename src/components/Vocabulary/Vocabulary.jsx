...
function StatCard({ label, value, tone = 'default' }) {
  const toneMap = {
    default: {
      bg: 'bg-white',
      icon: 'bg-neutral-50 text-neutral-400 border-neutral-100',
  ...
  return (
    <div
      className={`${currentTheme.bg} rounded-[2rem] p-6 lg:p-8 shadow-soft border ${tone === 'default' ? 'border-neutral-100' : 'border-transparent'} flex flex-col justify-between relative overflow-hidden group`}
    >
      {tone !== 'default' && (
      ...
      <div
        className={`text-[11px] font-bold uppercase tracking-widest mb-4 opacity-80 ${currentTheme.text}`}
      >
        {label}
      </div>
      <div className={`text-4xl md:text-5xl font-black ${currentTheme.text} relative z-10`}>
        {value}
      </div>
    </div>
  )
    ...
    <div className="bg-white rounded-2xl p-5 md:p-6 border border-neutral-100 shadow-sm hover:border-violet-200 hover:shadow-soft transition-all group flex flex-col gap-4">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-3">
          <div className="text-xl md:text-2xl font-extrabold tracking-tight text-neutral-900 group-hover:text-violet-700 transition-colors">
            {word.word}
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <span
              className={`rounded-md px-2 py-1 text-[10px] font-bold uppercase tracking-widest border shadow-inner-soft ${WORD_STATUS_META[word.status].badgeClass}`}
            >
        ...
        <button
          className="text-[11px] font-bold text-red-500 hover:text-white hover:bg-red-500 border border-red-200 hover:border-red-500 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 self-start"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-neutral-400 font-semibold pt-2 border-t border-neutral-50/80">
        <span className="uppercase tracking-widest text-[10px]">
          {word.source || 'manual'}
          {word.isSeeded ? ' · seeded' : ''}
        </span>
        <strong className="font-bold text-neutral-600 bg-white px-2 py-1 rounded-md border border-neutral-100 shadow-sm">
          ✅ {word.correctCount ?? word.dragCorrectCount ?? 0}{' '}
          <span className="mx-1 opacity-30 text-neutral-300">|</span> ❌{' '}
          {word.errorCount ?? word.dragWrongCount ?? 0}
        </strong>
      </div>
    </div>
  )
...
export default function Vocabulary() {
  ...
  const { words, addWord, addManyWords, importSeedWords, removeUnstudiedSeedWords, removeWord } =
    useWordStore()
  ...
  const collocationCount = useMemo(
    () => words.filter((item) => item.entryType === 'collocation').length,
    [words],
  )
    ...
    <div className="text-neutral-800 antialiased min-h-screen flex flex-col pt-0 lg:pt-0 pb-16">
        ...
        <div className="flex items-center gap-8">
          <div>
            ...
            <p className="text-xs font-semibold tracking-wider text-neutral-400 uppercase mt-0.5">
              Gerencie palavras, collocations, seed NGSL e o banco que alimenta Reader, Builder e
              revisão.
            </p>
          </div>
        </div>
      ...
      <main className="max-w-[1400px] w-full mt-2 lg:mt-4 mx-auto px-4 md:px-8">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
          ...
          <StatCard
            label="Ativas"
            value={words.filter((item) => item.status === 'ativa').length}
            tone="orange"
          />
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[400px_minmax(0,1fr)]">
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 shadow-soft border border-neutral-100 flex flex-col relative overflow-hidden">
              ...
              <div className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1 relative z-10">
                Adicionar manualmente
              </div>
              <h3 className="text-xl font-bold text-neutral-900 mb-6 relative z-10">
                Entrada rápida
              </h3>
              ...
              <div className="space-y-4 relative z-10">
                ...
                <select
                  className="w-full bg-neutral-50/50 border border-neutral-200 focus:border-violet-500 rounded-xl px-4 py-3 text-sm font-semibold outline-none shadow-inner-soft"
                  value={entryType}
                ...
                <button
                  className="w-full bg-neutral-900 text-white hover:bg-black px-6 py-3.5 rounded-xl font-bold shadow-sm transition-transform hover:-translate-y-0.5 flex items-center justify-center gap-2"
            ...
            <div className="bg-white rounded-3xl p-6 shadow-soft border border-neutral-100 flex flex-col relative overflow-hidden">
              ...
              <p className="text-xs font-semibold text-neutral-500 mb-4 leading-relaxed">
                Cole palavras ou expressões separadas por vírgula ou quebra de linha. Duplicados
                serão ignorados.
              </p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button
                    className="flex-1 bg-neutral-900 text-white hover:bg-black px-4 py-3 rounded-xl font-bold shadow-sm transition-transform hover:-translate-y-0.5 flex items-center justify-center gap-2"
                    onClick={handleBulkAdd}
                  >
                    <Upload className="h-4 w-4" /> Importar
                  </button>
                  <button
                    className="px-5 py-3 rounded-xl font-bold text-neutral-500 border border-neutral-200 hover:bg-neutral-50 transition-colors"
                    onClick={() => {
                      setBulkWords('')
                ...
                {bulkFeedback && (
                  <div
                    className={`mt-2 p-3 rounded-xl text-xs font-bold leading-tight flex items-center gap-2 ${bulkFeedback.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-orange-50 text-orange-700 border border-orange-200'}`}
                  >
                    {bulkFeedback.text}
                  </div>
                )}
            ...
            <div className="bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 opacity-60 pointer-events-none"></div>
              <div className="text-[10px] font-bold text-violet-200 uppercase tracking-widest mb-1 relative z-10">
                Banco seed
              </div>
              ...
              <div className="space-y-3 relative z-10 flex flex-col">
                <button
                  className="w-full bg-white text-violet-900 hover:bg-neutral-50 px-4 py-3.5 rounded-xl font-bold shadow-md transition-transform hover:-translate-y-0.5 flex items-center justify-start gap-3"
                  onClick={() => handleSeedImport(false)}
                >
                  <WandSparkles className="h-4 w-4 text-violet-500" />
                  Importar nível atual ({config.userLevel})
                </button>
                <button
                  className="w-full bg-white/10 text-white hover:bg-white/20 border border-white/20 px-4 py-3.5 rounded-xl font-bold transition-transform hover:-translate-y-0.5 flex items-center justify-start gap-3"
                  onClick={() => handleSeedImport(true)}
                >
                  <Download className="h-4 w-4" />
                  Importar até o nível atual
                </button>
                <button
                  className="w-full bg-black/20 text-white/90 hover:bg-black/30 text-xs mt-2 px-4 py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-2"
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-soft border border-neutral-100 flex flex-col">
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_200px] mb-8 pb-8 border-b border-neutral-100">
                ...
                <select
                  className="w-full min-h-[52px] bg-neutral-50/50 border border-neutral-200 focus:border-violet-500 rounded-2xl px-4 text-sm font-semibold outline-none shadow-inner-soft cursor-pointer transition-colors"
                  value={filter}
                ...
                {filteredWords.length > 0 ? (
                  ...
                  <div className="bg-neutral-50 border border-neutral-200/60 border-dashed rounded-[2rem] p-12 text-center flex flex-col items-center">
                    ...
                    <p className="text-sm font-semibold text-neutral-400">
                      Tente buscar por outras palavras ou remover os filtros ativos.
                    </p>
                  </div>
                )}
              </div>
