...
describe('srs helpers', () => {
  ...
  it('pushes builder words into training and later to active', () => {
    expect(getStatusAfterBuilder({ status: 'reconhecida', correctCount: 0, reviewCount: 0 }, { correct: true })).toBe('em_treino');
  ...
  it('demotes after a failed review and sorts due cards by delay then ease factor', () => {
    ...
    expect(getStatusAfterReview({ status: 'ativa', correctCount: 4, reviewCount: 5, recentLapses: 0, easeFactor: 2.4 }, 'bom')).toBe('dominada');
    expect(getStatusAfterReview({ status: 'ativa', correctCount: 4, reviewCount: 5, recentLapses: 1, easeFactor: 2.4 }, 'nao_lembro')).toBe('em_treino');
