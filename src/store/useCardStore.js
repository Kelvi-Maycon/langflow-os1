    ...
    (set, get) => ({
      ...
      addSentences: (sentenceList) => {
        const existingKeys = new Set(
          get().sentences.map(
            (sentence) => `${sentence.wordId}:${sentence.type}:${sentence.english}`,
          ),
        )
        const newSents = sentenceList
          .filter(
            (sentence) =>
              !existingKeys.has(`${sentence.wordId}:${sentence.type}:${sentence.english}`),
          )
          .map((s) => ({
            id: uuid(),
            wordId: s.wordId,
      ...
      getSentencesByWord: (wordId) => get().sentences.filter((s) => s.wordId === wordId),

      getRecentSentencesByWord: (wordId, limit = 3) =>
        get()
          .sentences.filter((sentence) => sentence.wordId === wordId)
      ...
      reviewCard: (id, rating) => {
        ...
        set((state) => ({
          flashcards: state.flashcards.map((f) =>
            f.id === id ? { ...f, ...srsResult, reviewCount: (f.reviewCount || 0) + 1 } : f,
          ),
        }))
        return srsResult
      },
      ...
      getDueCardsSorted: (limit = Infinity) =>
        sortDueCards(get().flashcards.filter(isDueToday)).slice(0, limit),
